...
int main(){
    ...
    for(int i=0;i<custom.size();i++){
        ...
        if(endTime[idx]<=custom[i].comeTime){
            ...
            toTime+=(endTime[idx]-custom[i].comeTime);
            endTime[idx]+=custom[i].serveTime;
        }
    }
    if(custom.size()==0)printf("0.0");
    else{
        printf("%.1f",toTime/60.0/custom.size());
    }
    return 0;
}